...
calculateStats <- function(df, ds1, ds2, column, subsampleSize,
  ...
  ## 计算Multi-KS与KDE
  if(length(column) == 2)  {
    MultiKS <- fasano.franceschini.test::fasano.franceschini.test(df[df$dataset == ds1, column],df[df$dataset == ds2, column])
    KDE <- ks::kde.test(df[df$dataset == ds1, column],df[df$dataset == ds2, column])
  }
  ...
  if (length(column) == 1)
    ...
    c(NNmismatch = signif(mean(chisilh[, "chisqp"]), 3),
      avesilh = signif(mean(chisilh[, "silh"]), 3),
      avesilhlocal = signif(mean(chisilh[, "silhlocal"]), 3),
      MultiKSstatistic = MultiKS$statistic,
      MultiKSpvalue = MultiKS$p.value,
      KDETstat = KDE$Tstat,
      KDEZstat = KDE$zstat,
      KDEpvalue = KDE$pvalue)
}
  ...
  if (n == 1) {
    ...
    if (withP) {
      c(NNmismatch = NA_real_, avesilh = NA_real_, avesilhlocal = NA_real_,
        ...
        MultiKSstatistic = NA_real_, MultiKSpvalue = NA_real_, KDETstat = NA_real_,
        KDEZstat = NA_real_, KDEpvalue = NA_real_)
    } else {
      c(NNmismatch = NA_real_, avesilh = NA_real_, avesilhlocal = NA_real_,
        MultiKSstatistic = NA_real_, MultiKSpvalue = NA_real_,KDETstat = NA_real_,
        KDEZstat = NA_real_, KDEpvalue = NA_real_)
